...
static int set_parity( c_serial_port_t* desc, enum CSerial_Parity parity ) {
    ...
    } else if( parity == CSERIAL_PARITY_MARK ) {
        newio.Parity = MARKPARITY;
    } else if( parity == CSERIAL_PARITY_SPACE ) {
        newio.Parity = SPACEPARITY;
    }
#else
    ...
    if( parity == CSERIAL_PARITY_NONE ) {
        ...
        newio.c_cflag |= ( PARENB | CMSPAR );
    } else if( parity == CSERIAL_PARITY_SPACE ) {
        newio.c_cflag |= ( PARENB | CMSPAR | PARODD );
    }
#endif
...
int c_serial_new( c_serial_port_t** port, c_serial_errnum_t* errnum ) {
    ...
    new_port->baud_rate = CSERIAL_BAUD_115200;
    ...
    memset(&(new_port->async_IO_operation), 0, sizeof(OVERLAPPED) );
    new_port->async_IO_operation.hEvent = CreateEvent(0, FALSE, FALSE, 0 );
#else
...
int c_serial_write_data( c_serial_port_t* port, void* data, unsigned length, unsigned * bytes_written )
{
#ifdef _WIN32
...
#ifdef _WIN32
    if( !WriteFile( port->port, data, length, NULL, &(port->async_IO_operation) ) ) {
        port->last_errnum = GetLastError();
        if( GetLastError() == ERROR_IO_PENDING ) {
            /* Probably not an error, we're just doing this in an async fasion */
            if(WaitForSingleObject(port->async_IO_operation.hEvent, INFINITE ) == WAIT_FAILED )
            {
                port->last_errnum = GetLastError();
	...
	if(GetOverlappedResult(port->port, &(port->async_IO_operation), &bytes_written2, 1 ) == 0 ){
		LOG_ERROR( LOGGER_NAME, "Unable to write data" );
		return CSERIAL_ERROR_GENERIC;
	}
#else
...
int c_serial_read_data( c_serial_port_t* port, void* data, unsigned data_length, unsigned * bytes_read, c_serial_control_lines_t* lines )
{
	...
	do{
		...
		if( !current_available ){
			...
			WaitCommEvent( port->port, &ret, &(port->async_IO_operation) );
			WaitForSingleObject(port->async_IO_operation.hEvent, INFINITE );
		}
		else{
		...
		if( ret & EV_RXCHAR && data != NULL ){
			if( !ReadFile( port->port, data, data_length, &bytesGot, &(port->async_IO_operation) ) ){
...
int c_serial_read_data_blocking (c_serial_port_t* port, void* data, unsigned data_length, unsigned *bytes_read)
{
#ifdef _WIN32
    OVERLAPPED ol;
    DWORD ret = 0;
    ...
    if( ! (ret = ReadFile( port->port, data, data_length, &bytesGot, &ol) ) ){
        LOG_ERROR( "Unable to read bytes from port", port );
        *bytes_read = 0;
    }

    if (GetLastError() == 997)
        ret = GetOverlappedResult(port, &ol, (DWORD *)bytes_read, 1);
...
int c_serial_write_data_blocking (c_serial_port_t* port, void* data, unsigned data_length, unsigned *bytes_written)
{
#ifdef _WIN32
    OVERLAPPED ol;
    DWORD ret = 0;
    ...
    if( ! ( ret = WriteFile( port->port, data, data_length, &bytesGot, &ol) ) )
    {
        LOG_ERROR( "Unable to write bytes to port", port);
        printf("ERROR CODE: %li", port->last_errnum);
        port->last_errnum = GetLastError();
        printf("ERROR CODE: %li", port->last_errnum);
        *bytes_written = 0;
    }

    if (port->last_errnum == 997)
        ret = GetOverlappedResult(port, &ol, (DWORD *)bytes_written, 1);

    *bytes_written = bytesGot;
...
c_serial_handle_t c_serial_get_poll_handle( c_serial_port_t* port ){
    ...
    return port->async_IO_operation.hEvent;
#else
    CHECK_INVALID_PORT( port );
    return port->port;
#endif
